...
/* ═══════════════════════════════════════════════════════════════
   ...
   Parameter keys match the standalone effect. The MediaPipe
   subject mask (sharp-subject gating) stays iframe-only for now:
   the node treats the whole frame as background.
   ═══════════════════════════════════════════════════════════════ */
...
void main(){
  ...
  if(uStyle == 0){
    ...
    for(int i=0;i<7;i++){
      ...
      t = vUV + vec2(cos(a)*rr*px.x, sin(a)*rr*px.y*uAspect);
      tapAcc(t, 1.0, bloomK*1.3, acc, wt); /* outer ring: stronger highlight boost */
    }
  }
  ...
  /* STYLE 2: EXPLOSIVE — tight inner disc + radial burst streaks */
    ...
    for(int i=0;i<12;i++){
      ...
      tapAcc(vUV + vec2(cos(a)*r*px.x, sin(a)*r*px.y*uAspect), 1.0, bloomK, acc, wt);
    }
    for(int i=0;i<8;i++){
      float a = float(i)*0.7854 + gold(vUV, float(i)*2.2+uTime*0.013)*0.1;
      for(int k=0;k<4;k++){
        float r = mix(coc*0.9, coc*2.0, float(k)/3.0);
        tapAcc(vUV + vec2(cos(a)*r*px.x, sin(a)*r*px.y*uAspect), 1.0, bloomK*(1.0+float(k)*0.4), acc, wt);
      }
    }
  }
  ...
  /* STYLE 3: ANAMORPHIC 2.39:1 — elliptical rings + horizontal streak taps */
    ...
    for(int i=1;i<=8;i++){
      ...
      tapAcc(vUV+vec2( xOff,0.0), fw,  bloomK, acc, wt);
      tapAcc(vUV+vec2(-xOff,0.0), fw2, bloomK, acc, wt);
    }
  }
...
export class BokehNode implements EngineNode {
  ...
  init(gl: WebGL2RenderingContext): void {
    ...
    this.locate(gl, 'bokeh', this.bokehProg, ['uTex', 'uRadius', 'uBloom', 'uStyle', 'uPx', 'uAspect', 'uTime']);
